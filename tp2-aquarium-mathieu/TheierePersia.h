...
public:
    Theiere(float r = 1.0, float h = 0.0, float ang = 0.0, float vit = 1.0, float tai = 0.5, bool sel = false)
        : rayon(r), hauteur(h), angle(ang), vitesse(vit), taille(tai), estSelectionnee(sel)
    {
    ...
    void afficher()
    {
        matrModel.PushMatrix(); { // sauvegarder la tranformation courante
            ...
            if (Etat::enSelection) { glVertexAttrib3fv(locColor, glm::value_ptr(couleurSel)); }
    ...
    {
        if (!estSelectionnee)  // si rien n'est sélectionné
        {
